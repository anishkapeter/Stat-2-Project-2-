---
title: "Proj2"
author: "Kyle Kuberski, Anishka Peter, Haitie Liu"
date: "2023-11-26"
output:
...
options(scipen=999)
# Train Data Set 
data <- read.table("https://raw.githubusercontent.com/anishkapeter/Stat-2-Project-2-/main/adult.data", header = FALSE, sep = ",")
# rename cols
...
# Train Dataset 
validate <- read.csv("https://raw.githubusercontent.com/anishkapeter/Stat-2-Project-2-/main/adult.test",header=FALSE)
validate <- validate[-1,]
colnames(validate) <- c("age", "workclass", "fnlwgt", "education", "education_num", 
                         "marital_status", "occupation", "relationship", "race", "sex", 
                         "capital_gain", "capital_loss", "hours_per_week", "native_country", "income")
# change age into int
validate$age <- as.integer(validate$age)
#change ? data in workclass/occupation to UNKNOWN
validate$workclass <- ifelse(validate$workclass == " ?", "UNKNOWN", validate$workclass)
validate$occupation <- ifelse(validate$occupation == " ?", "UNKNOWN", validate$occupation)


#Factoring Categoricals/Checking References
validate$workclass <- as.factor(validate$workclass)
levels(validate$workclass)

validate$education <- as.factor(validate$education)
levels(validate$education)

validate$marital_status <- as.factor(validate$marital_status)
levels(validate$marital_status)

validate$occupation <- as.factor(validate$occupation)
levels(validate$occupation)

validate$relationship <- as.factor(validate$relationship)
levels(validate$relationship)

validate$race <- as.factor(validate$race)
levels(validate$race)

validate$sex <- as.factor(validate$sex)
levels(validate$sex)

#Factor our response
validate$income <- as.factor(validate$income)
